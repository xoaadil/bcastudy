  ...
  useEffect(() => {
    const fetchCoding = async () => {
      try {
        ...
        console.error(err);
        setError("Failed to load coding resources");
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="w-full min-h-screen bg-white flex flex-col items-center justify-center py-10 px-4 relative overflow-hidden">
        ...
        <div className="relative z-10 text-center">
          <div className="relative mb-8">
            <div className="w-20 h-20 bg-gradient-to-br from-violet-600 to-purple-800 rounded-3xl flex items-center justify-center shadow-2xl mx-auto animate-pulse">
              <Code className="w-10 h-10 text-white" />
            </div>
            <div className="absolute inset-0 bg-violet-500/20 rounded-3xl blur-xl animate-pulse"></div>
          </div>
          <p className="text-xl text-gray-600 font-medium mb-6">Loading coding resources...</p>
          <div className="flex justify-center space-x-2">
            <div className="w-3 h-3 bg-violet-500 rounded-full animate-bounce"></div>
            <div className="w-3 h-3 bg-purple-600 rounded-full animate-bounce" style={{animationDelay: '0.1s'}}></div>
            <div className="w-3 h-3 bg-indigo-700 rounded-full animate-bounce" style={{animationDelay: '0.2s'}}></div>
          </div>
        </div>
      </div>
    );
    ...
    <div className="w-full min-h-screen bg-white flex flex-col items-center py-10 px-4 relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-40">
        <svg className="w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
          <defs>
            <linearGradient id="codingBlueWave" x1="0%" y1="0%" x2="100%" y2="100%">
              <stop offset="0%" stopColor="#3b82f6" stopOpacity="0.05" />
              <stop offset="50%" stopColor="#1d4ed8" stopOpacity="0.03" />
              <stop offset="100%" stopColor="#3b82f6" stopOpacity="0.05" />
            </linearGradient>
          </defs>
          <path d="M0,60 Q25,40 50,60 T100,60 L100,100 L0,100 Z" fill="url(#codingBlueWave)">
            <animateTransform
      ...
      <div className="relative z-10 w-full max-w-6xl">
        ...
        <div className={`text-center mb-20 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <div className="inline-flex items-center gap-4 mb-8 group">
            <div className="w-16 h-16 bg-gradient-to-br from-violet-600 to-purple-800 rounded-2xl flex items-center justify-center shadow-2xl group-hover:shadow-violet-500/30 transition-all duration-500 group-hover:rotate-12">
              <Code className="w-8 h-8 text-white" />
            </div>
            <h1 className="text-5xl font-bold bg-gradient-to-r from-gray-900 via-violet-900 to-black bg-clip-text text-transparent">
              Master Programming & Development
            </h1>
          </div>

          <p className="text-xl text-gray-600 max-w-4xl mx-auto leading-relaxed mb-8">
            Build real-world skills with hands-on coding practice, expert guidance, and 
            comprehensive resources for programming excellence.
          </p>

          <div className="w-40 h-0.5 bg-gradient-to-r from-transparent via-violet-500 to-transparent mx-auto"></div>
        </div>

        {/* Coding Resources Grid */}
        <div className={`grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 max-w-5xl mx-auto mb-12 lg:mb-16 transition-all duration-1000 delay-300 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-20'}`}>
          {coding.map((item, index) => (
            ...
            >
              ...
              {/* Main Card */}
              <div className="relative bg-white rounded-2xl lg:rounded-3xl p-6 lg:p-8 border border-gray-200 hover:border-violet-300 transition-all duration-500 shadow-xl hover:shadow-2xl group-hover:shadow-violet-500/20 h-full">
                ...
                <div className="flex items-center justify-between mb-6">
                  <div className={`w-12 h-12 lg:w-14 lg:h-14 bg-gradient-to-br ${gradientColors[index % gradientColors.length]} text-white rounded-2xl flex items-center justify-center shadow-xl group-hover:shadow-2xl transition-all duration-500 group-hover:rotate-6 flex-shrink-0`}>
                    <Code className="w-6 h-6 lg:w-7 lg:h-7" />
                  </div>
                  <a
                    href={item.link}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-violet-600 group-hover:text-violet-700 transition-colors duration-300 transform group-hover:translate-x-1 lg:group-hover:translate-x-2"
                  >
                ...
                {/* Title */}
                <h2 className="text-lg lg:text-xl xl:text-2xl font-bold text-gray-900 mb-4 lg:mb-6 group-hover:text-violet-700 transition-colors duration-300 line-clamp-2 break-words">
                  {item.title}
                </h2>
                ...
                <div className="mb-6">
                  ...
                  {item.description.length > 120 && (
                    <button
                      ...
                      className="text-violet-600 hover:text-violet-700 text-sm mt-2 transition-colors duration-300 font-medium"
                    >
                ...
                <div className="flex items-center gap-2 mb-6 flex-wrap">
                  <div className="flex items-center gap-1 bg-violet-50 px-2 py-1 rounded-lg border border-violet-200">
                    <Target className="w-3 h-3 text-violet-600 flex-shrink-0" />
                    <span className="text-xs text-violet-700 font-medium">Expert</span>
                  </div>
                  ...
                  <div className="flex items-center gap-1 bg-green-50 px-2 py-1 rounded-lg border border-green-200">
                    ...
                    <span className="text-xs text-green-700 font-medium">Hands-on</span>
                  </div>
                </div>
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-2 text-violet-600 group-hover:text-violet-700 transition-colors duration-300 font-semibold"
                >
                  <span className="text-sm lg:text-base truncate">Explore Resources</span>
                ...
                {/* Decorative Elements */}
                <div className="absolute -top-2 -right-2 lg:-top-3 lg:-right-3 w-4 h-4 lg:w-6 lg:h-6 bg-violet-500 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500 animate-pulse shadow-lg"></div>
        ...
        <div className={`text-center transition-all duration-1000 delay-600 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-20'}`}>
          <div className="bg-gradient-to-r from-gray-50 to-white backdrop-blur-xl rounded-3xl p-12 border border-gray-200 shadow-2xl hover:shadow-3xl transition-all duration-500 max-w-2xl mx-auto">
            ...
            <Link 
              to="/"
              className="group relative inline-flex items-center gap-3 px-10 py-4 bg-gradient-to-r from-violet-700 to-purple-800 text-white font-semibold rounded-2xl transition-all duration-300 hover:scale-105 hover:-translate-y-1 shadow-2xl hover:shadow-violet-500/40"
            >
              <span className="relative z-10 flex items-center gap-3">
                <Home className="w-5 h-5 group-hover:-translate-x-1 transition-transform duration-300" />
                Back to Home
              </span>
              <div className="absolute inset-0 bg-gradient-to-r from-violet-600 to-purple-700 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
            </Link>
          </div>
        </div>
      </div>
      ...
      <div className="absolute top-20 right-20 w-3 h-3 bg-violet-500/30 rounded-full shadow-lg"></div>
      <div className="absolute bottom-20 left-20 w-4 h-4 bg-gray-400/20 rounded-full shadow-lg"></div>
      <div className="absolute top-1/2 right-32 w-2 h-2 bg-violet-600/40 rounded-full shadow-lg"></div>
    </div>
  );
}